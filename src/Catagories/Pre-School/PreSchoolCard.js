...
import Images from './PreSchoolImages';
import CourseInfo from './PreSchoolCourseInfo'
...
<div className="s_cat_div">
    ...
    {/* {this.props.sub.courses.map(course => < Images course={course}/>)} */}
     ...
     <Switch>
           <Redirect to={{
                pathname: `/pre_school/${this.state.clickedcourse.id}`,
                state: { clickedcourse: this.state.clickedcourse.id, clickedcourse: this.state.clickedcourse}
            }} />
            <Route path={`/pre_school/${this.state.clickedcourse.id}`} >
