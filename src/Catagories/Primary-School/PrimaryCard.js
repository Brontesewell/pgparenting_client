...
import Images from './PrimaryImages';
import CourseInfo from './PrimaryCourseInfo'
...
<div className="s_cat_div">
    ...
    {/* {this.props.sub.courses.map(course => < Images course={course}/>)} */}
     ...
     <Switch>
           <Redirect to={{
                pathname: `/primary_school/${this.state.clickedcourse.id}`,
                state: { clickedcourse: this.state.clickedcourse.id, clickedcourse: this.state.clickedcourse}
            }} />
            <Route path={`/primary_school/${this.state.clickedcourse.id}`} >
