...
import Images from './ToddlerImages';
import CourseInfo from './ToddlerCourseInfo'
...
<div className="s_cat_div">
    ...
    {/* {this.props.sub.courses.map(course => < Images course={course}/>)} */}
     ...
     <Switch>
           <Redirect to={{
                pathname: `/toddler/${this.state.clickedcourse.id}`,
                state: { clickedcourse: this.state.clickedcourse.id, clickedcourse: this.state.clickedcourse}
            }} />
            <Route path={`/toddler/${this.state.clickedcourse.id}`} >
